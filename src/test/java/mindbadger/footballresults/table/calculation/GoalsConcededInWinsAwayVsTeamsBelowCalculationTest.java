...
import mindbadger.footballresults.table.calculation.GoalsConcededInWinsAwayVsTeamsBelowCalculation;
...
public class GoalsConcededInWinsAwayVsTeamsBelowCalculationTest {
	private GoalsConcededInWinsAwayVsTeamsBelowCalculation objectUnderTest;
	...
	public void shouldReturnNoChangeWhenThereIsAPreviousRowForAHomeWinAgainstATeamAbove () {
		...
		when (mockPreviousTableRow.getAttribute(GOALS_CONCEDED_IN_WINS_AWAY_VS_BELOW)).thenReturn(3);
		
		objectUnderTest = new GoalsConcededInWinsAwayVsTeamsBelowCalculation(mockTeamForCalculation, mockFixture, mockFixtureTeamContext, mockOppositionTeamContext, mockPreviousTableRow);
	...
	@Test
	public void shouldReturnAnIncrementWhenThereIsAPreviousRowForAnAwayWinAgainstATeamBelow () {
		// Given
		when (mockFixtureTeamContext.isAtHome()).thenReturn(false);
		when (mockFixtureTeamContext.isPlayingTeamAbove()).thenReturn(false);
		when (mockFixture.getHomeGoals()).thenReturn(1);
